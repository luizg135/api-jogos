...
# --- NOVAS ROTAS PARA SINCRONIZAÇÃO STEAM ---
@game_bp.route('/sync/steam', methods=['POST'])
@jwt_required()
def sync_steam_library():
    """
    Sincroniza a biblioteca de jogos do usuário da Steam.
    Recebe o Steam ID e uma lista de jogos selecionados para sincronizar.
    """
    data = request.json
    steam_id = data.get('steam_id')
    selected_games = data.get('selected_games', []) # Lista de jogos selecionados pelo frontend

    if not steam_id:
        return jsonify({"success": False, "message": "Steam ID é obrigatório."}), 400

    try:
        result = game_service.sync_steam_library(steam_id, selected_games)
        return jsonify(result)
    except Exception as e:
        print(f"ERRO NA ROTA /sync/steam: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "message": "Erro ao sincronizar biblioteca Steam.", "detalhes_tecnicos": str(e)}), 500

@game_bp.route('/sync/steam/preview', methods=['GET'])
@jwt_required()
def preview_steam_library():
    """
    Retorna uma prévia dos jogos da Steam do usuário para seleção no frontend.
    """
    ...
    try:
        preview_data = game_service.get_steam_library_preview(steam_id)
        return jsonify(preview_data)
    except Exception as e:
        print(f"ERRO NA ROTA /sync/steam/preview: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "message": "Erro ao obter prévia da biblioteca Steam.", "detalhes_tecnicos": str(e)}), 500
# --- FIM NOVAS ROTAS PARA SINCRONIZAÇÃO STEAM ---

# --- NOVA ROTA PARA HISTÓRICO DE PREÇOS ---
@game_bp.route('/wishlist/price-history/<string:game_name>', methods=['GET'])
@jwt_required()
def get_wishlist_price_history(game_name):
    """Retorna o histórico de preços de um item da lista de desejos."""
    try:
        history = game_service.get_price_history(game_name)
        return jsonify(history)
    except Exception as e:
        print(f"ERRO NA ROTA /wishlist/price-history/{game_name}: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "message": "Erro ao buscar histórico de preços.", "detalhes_tecnicos": str(e)}), 500
# --- FIM NOVA ROTA PARA HISTÓRICO DE PREÇOS ---

# --- NOVA ROTA PARA SORTEAR JOGO ---
@game_bp.route('/random-game', methods=['GET'])
@jwt_required()
def get_random_game():
    """
    Retorna um jogo aleatório da biblioteca com base em filtros.
    Filtros opcionais: plataforma, estilo, min_metacritic, max_metacritic.
    """
    platform = request.args.get('platform')
    style = request.args.get('style')
    min_metacritic = request.args.get('min_metacritic', type=int)
    max_metacritic = request.args.get('max_metacritic', type=int)

    try:
        random_game = game_service.get_random_game(platform, style, min_metacritic, max_metacritic)
        if random_game:
            return jsonify({"success": True, "game": random_game})
        else:
            return jsonify({"success": False, "message": "Nenhum jogo encontrado com os critérios fornecidos."}), 404
    except Exception as e:
        print(f"ERRO NA ROTA /random-game: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "message": "Erro ao sortear jogo.", "detalhes_tecnicos": str(e)}), 500
# --- FIM NOVA ROTA PARA SORTEAR JOGO ---

# --- NOVA ROTA PARA JOGOS SIMILARES ---
@game_bp.route('/similar/<int:rawg_id>', methods=['GET'])
@jwt_required()
def get_similar_games_route(rawg_id):
    """
    Retorna jogos similares a um determinado RAWG_ID, excluindo jogos já possuídos.
    """
    try:
        similar_games = game_service.get_similar_games(rawg_id)
        return jsonify(similar_games)
    except Exception as e:
        print(f"ERRO NA ROTA /similar/{rawg_id}: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "message": "Erro ao buscar jogos similares.", "detalhes_tecnicos": str(e)}), 500
# --- FIM NOVA ROTA PARA JOGOS SIMILARES ---
